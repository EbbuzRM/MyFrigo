...
async def run_test():
    ...
    try:
        ...
        # Check if we were redirected to dashboard/inventory
        current_url = page.url
        print(f"Current URL after login attempt: {current_url}")
        
        # Basic validation - check if login appears successful
        if "dashboard" in current_url.lower() or "inventory" in current_url.lower() or "products" in current_url.lower():
            print("Login appears to have succeeded")
        elif "error" in current_url.lower() or "login" in current_url.lower():
